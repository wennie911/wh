...
void CCompanyQANormal::initNA(vector<struWordList> vecWordsList, int iTone, void *vPtr, void *vApplyLocatorPt)
{
	...
	vector<CINSemanticInfo*>::const_iterator iter = ((CINSemanticInfos *)vPtr)->m_vSemantics.begin();
	m_iTone = (*iter)->m_nTone;
	int iToneEx = (*iter)->m_nToneEx;
	m_sTone = "";
	if((*iter)->isNegativeTone()) //否定句
	{
		m_sTone = "neg";
	}
	if((*iter)->isQuestionTone())//疑问句
	{
		//1.when;2.where;3.who;4.how;5.what;6.which;7.why
		switch(iToneEx)
		{
			case 1:
				m_sTone="when";
...
bool CCompanyQANormal::getAnswer(string & retStr)
{
	...
	qaDefault.media.sQuestionType = m_sTone;
...
string CCompanyQANormal::getAnswerFromId()
{
	...
	if(vecIdList.size()==1)//只有一个ID的时候,认为是唯一答案
	{
		...
		qa.media.sQuestionType = m_sTone;
