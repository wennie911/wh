...
bool CFilterProcess::init(string &sDllDir)
{
	...
	printf("%s,%s\n",sDllPath.c_str(), sDataPath.c_str());
...
bool CFilterProcess::processInSentence(string &sInSentence, string &sOutSentence )
{
	...
	retAnswer["questionType"] =Json::Value(m_retQA.sQuestionType);
...
bool CFilterProcess::readFilterConfig(string sXMLPath)
{
	...
	while(pLayer!=NULL)
	{
		...
		layer.sDefDenyQuestion = getXmlAttribute(pLayer,"defDenyQ");
		layer.sDefQuestion = getXmlAttribute(pLayer,"defQ");
		layer.sDefWhenQuestion = getXmlAttribute(pLayer,"defWhenQ");
		layer.sDefWhereQuestion = getXmlAttribute(pLayer,"defWhereQ");
		layer.sDefWhoQuestion = getXmlAttribute(pLayer,"defWhoQ");
		layer.sDefHowQuestion = getXmlAttribute(pLayer,"defHowQ");
		layer.sDefWhatQuestion = getXmlAttribute(pLayer,"defWhatQ");
		layer.sDefWhichQuestion = getXmlAttribute(pLayer,"defWhichQ");
		layer.sDefWhyQuestion = getXmlAttribute(pLayer,"defWhyQ");
...
bool CFilterProcess::layerProcess()
{
	...
	if(!vecAllowQAList.empty())
	{
		...
		//保存回答的answer
		//printf("sQuestiontype=%s\n", iterQA->sQuestionType.c_str());
		setRetAnswer(*iterQA);
	...
	else if(!vecOtherQAList.empty())
	{
			...
			if(iterOtherQA->sAnswer.compare("@sayagain")==0)
			{
				...
				if(sAnswer.length()==0)
				{
					...
					setRetAnswer(sAnswer, sAudio, 0, iterOtherQA->sQuestion, iterOtherQA->sMatchedQuestion, 
						iterOtherQA->sQuestionPinYin,iterOtherQA->sType, iterOtherQA->sQuestionType);
		...
		for(vector<struNLUQA>::iterator iterOtherQA = vecOtherQAList.begin(); iterOtherQA != vecOtherQAList.end(); iterOtherQA++)
		{
			if(iterOtherQA->iAffirm==0 || iterOtherQA->sQuestionType.compare("neg")==0)
			{
				printf("affirm\n");
				//调用一次nlu 把缺省的问题发一遍
		...
		//如果qalist有疑问句类型, 判断过滤层是否有相关回答的Q
		for(vector<struNLUQA>::iterator iterOtherQA = vecOtherQAList.begin(); iterOtherQA != vecOtherQAList.end(); iterOtherQA++)
		{
			if(iterOtherQA->sQuestionType.size()>0)
			{
				string sDefQuestionW;
				string sQuestionType = iterOtherQA->sQuestionType;
				if(sQuestionType.compare("when")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefWhenQuestion();
				}
				else if(sQuestionType.compare("where")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefWhereQuestion();
				}
				else if(sQuestionType.compare("who")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefWhoQuestion();
				}
				else if(sQuestionType.compare("how")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefHowQuestion();
				}
				else if(sQuestionType.compare("what")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefWhatQuestion();
				}
				else if(sQuestionType.compare("which")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefWhichQuestion();
				}
				else if(sQuestionType.compare("why")==0)
				{
					sDefQuestionW = oneFilterPtr->getDefWhyQuestion();
				}
				...
				vector<struNLUQA> vecNLUQAListT;
				vector<struNLUQA>::iterator iterQA;
				if(!m_so_nlu.getAnswer(sDefQuestionW, vecNLUQAListT))
					return false;
				iterQA=vecNLUQAListT.begin();
...
void CFilterProcess::setRetAnswer(struNLUQA & oneQA)
{
	...
	m_retQA.sQuestionType = oneQA.sQuestionType;
	m_retQA.sType = oneQA.sType;
	m_retQA.iIsBreak = oneQA.iIsBreak;
	m_retQA.iIsEnd = oneQA.iIsEnd;
}

void CFilterProcess::setRetAnswer(string &sAnswer, string &sAudio, int iIsEnd, string &sQuestion, string &sMatchedQuestion, string &sQuestionPinYin, string &sType, string &sQuestionType)
{
	printf("set questionType = %s\n", sQuestionType.c_str());
	...
	m_retQA.sQuestionType = sQuestionType;
	m_retQA.sType = sType;
	m_retQA.iIsBreak = 1;
